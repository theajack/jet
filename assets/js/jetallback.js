...
(function(){
  ...
  Jet.prototype.$route=function(s){
    Jet.router.route(s);
  };
  Jet.prototype.$=_JT;
  Jet.$=_JT;
...
//
  ...
  function _loadStyle(out){
    ...
    if(index==-1){
      style._JT_html(commonCss.join('')+txt.join(''));
    }
  }
  ...
  function _loadCompScript(out,attr){
    ...
    if(!script._JT_exist()){
      ...
      scripts._JT_each(function(item,i){
        if(item._JT_hasAttr("src")){
          _JT.load(Jet.router.conf.js+item._JT_attr("src"),function(src){
            txt[i+1]=src;
            if(i==index){
              script._JT_html(txt.join(';'));
              _JT.body()._JT_append(script);
