  ...
  Jet.Base=function(opt,type){
    ...
    if(arguments.length==2){
      if(name._JT_has('.')){
        ...
        for(var i=1;i<a.length;i++){
            _call=_checkIsIndex(_call,a[i])//_call[a[i]];
        }
        ...
        var _call=_checkIsIndex(this._tools._calls,name)
        _call._func.push(call);
      }
    }else{
...
Jet.Text.prototype.refresh=function(key){
  ...
  /*if*********************************************************************************/
  // Jif="exp:class[a,b|b];attr[a=b,a=b|a];func"
  ...
  function _registForWrapperVar(_this,content){
    ...
    if(m==null){
        if(!(content in _this.data)&&content!="$"){
        ...
        m.forEach(function(_ele){
            if(arr.indexOf(_ele)==-1){
                ...
                if(_ele=="{{$}}"){
                    ...
                    _this.regist(_ele.substring(2,_ele.length-2),function(key,val){
                        _this.refresh();
                    })
                }
            }
  ...
  function _initIf(){
    ...
    if(this.type==_show){
        ...
        this.exp=new Function("d","return ("+ifAttr+")");
    }else{
